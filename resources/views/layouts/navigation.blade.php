        ...
        <div class="flex justify-between items-center h-16 gap-2">
            ...
            <div class="flex items-center gap-1.5 sm:gap-3 flex-shrink-0">
                @if(Auth::check() && Auth::user()->role === 'masyarakat')
                    @php
                        $userNotifications = Auth::user()->permohonans()
                            ->whereIn('status', ['selesai', 'ditolak'])
                            ->orderBy('updated_at', 'desc')
                            ->take(10)
                            ->get();
                        $unreadCount = $userNotifications->filter(fn($p) => !$p->isNotifRead())->count();
                        $readIds = $userNotifications->filter(fn($p) => $p->isNotifRead())->pluck('id')->values()->all();
                    @endphp

                    <!-- Notification Bell Dropdown (Masyarakat Only) -->
                    <div class="relative" x-data="{ 
                        notifOpen: false,
                        unreadCount: {{ $unreadCount }},
                        readIds: {{ json_encode($readIds) }},
                        isRead(id) {
                        ...
                        markAsRead(id) {
                            if (!this.readIds.includes(id)) {
                                ...
                                fetch('{{ url('/masyarakat/permohonan') }}/' + id + '/mark-read', {
                                    method: 'POST',
                                    headers: {
                        ...
                        markAllAsRead() {
                            this.unreadCount = 0;
                            @foreach($userNotifications as $notifItem)
                                if (!this.readIds.includes({{ $notifItem->id }})) {
                                    this.readIds.push({{ $notifItem->id }});
                                }
                            @endforeach
                            fetch('{{ route('masyarakat.notif.markAll') }}', {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json',
                                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
                        ...
                        <button @click="notifOpen = !notifOpen"
                            ...
                            <template x-if="unreadCount > 0">
                                <span class="absolute -top-1 -right-1 flex h-4 w-4 items-center justify-center pointer-events-none">
                                    <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-red-400 opacity-75"></span>
                                    <span class="relative inline-flex rounded-full h-4 w-4 bg-red-500 text-white text-[9px] font-black items-center justify-center" x-text="unreadCount > 9 ? '9+' : unreadCount"></span>
                                </span>
                            </template>
                        </button>
                        ...
                        <div x-show="notifOpen"
                            ...
                            <div class="p-3.5 sm:p-4 bg-gradient-to-r from-slate-900 to-slate-800 text-white flex items-center justify-between">
                                ...
                                <div class="flex items-center gap-2">
                                    <template x-if="unreadCount > 0">
                                        <button @click="markAllAsRead()" class="text-[11px] text-emerald-300 hover:text-white underline font-medium">
                                            Tandai dibaca
                                        </button>
                                    </template>
                                    <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold" :class="unreadCount > 0 ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/30' : 'bg-white/20 text-white'" x-text="unreadCount > 0 ? unreadCount + ' Baru' : 'Semua Terbaca'">
                                    </span>
                                </div>
                            </div>
                            ...
                            <div class="max-h-[320px] overflow-y-auto divide-y divide-slate-100">
                                @forelse($userNotifications as $item)
                                    <div class="p-3.5 sm:p-4 transition-colors relative group"
                                        :class="!isRead({{ $item->id }}) ? 'bg-emerald-50/30 hover:bg-emerald-50/60 border-l-4 border-l-emerald-500' : 'bg-white hover:bg-slate-50'">
                                                ...
                                                <div class="flex-1 min-w-0">
                                                    <div class="flex items-center justify-between gap-1 mb-1">
                                                        <div class="flex items-center gap-1.5">
                                                            ...
                                                            <span x-show="!isRead({{ $item->id }})" class="w-1.5 h-1.5 rounded-full bg-emerald-500 inline-block flex-shrink-0" title="Belum dibaca"></span>
                                                        </div>
                                                    ...
                                                    <!-- WhatsApp forwarded indicator & Action button -->
                                                ...
                                                <div class="flex-1 min-w-0">
                                                    <div class="flex items-center justify-between gap-1 mb-1">
                                                        <div class="flex items-center gap-1.5">
                                                            ...
                                                            <span x-show="!isRead({{ $item->id }})" class="w-1.5 h-1.5 rounded-full bg-red-500 inline-block flex-shrink-0" title="Belum dibaca"></span>
                                                        </div>
                                                        <span class="text-[10px] text-slate-400">{{ $item->updated_at->diffForHumans() }}</span>
                                                    </div>
                                                    <p class="text-xs font-semibold text-slate-800 truncate">{{ $item->jenis_layanan }}</p>
